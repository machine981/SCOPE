...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # correct/wrong sample counts
    correct_mask = (sequence_score > 0)
    num_correct = correct_mask.sum().detach().item()
    num_wrong = (~correct_mask).sum().detach().item()
    total = sequence_score.shape[0]
    ...
    metrics = {
        ...
        **(
        ...
        # correct/wrong sample counts
        "batch/num_correct": num_correct,
        "batch/num_wrong": num_wrong,
        "batch/correct_ratio": num_correct / total if total > 0 else 0.0,
        # response length
