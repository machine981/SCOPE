...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_prob(self, data: DataProto, calculate_entropy=True):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with self.ulysses_sharding_manager:
            ...
            if len(compute_result) == 3:
                # top-k KL reward 模式：额外返回了 topk_kl (B, L)，覆盖 teacher_log_probs
                log_probs_out, entropys, topk_kl = compute_result
                output = DataProto.from_dict(
                    tensors={"old_log_probs": log_probs_out, "entropys": entropys, "teacher_log_probs": topk_kl},
                    meta_info={"temperature": self.config.rollout.temperature, "teacher_log_probs_is_topk_kl": True},
                )
            else:
                log_probs_out, entropys = compute_result
                output = DataProto.from_dict(
                    tensors={"old_log_probs": log_probs_out, "entropys": entropys},
